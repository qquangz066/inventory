...
const ViewProduct = ({ match }) => {
  const { detail, loading } = useSelector(state => state.product);
  ...
  useEffect(() => {
    ...
    if (match.params.id) {
      dispatch(productActions.get(match.params.id));
    } else if (match.params.upc) {
      dispatch(productActions.getByUpc(match.params.upc));
    }
  }, [dispatch, match.params.id, match.params.upc]);

  useEffect(() => {
    reset(detail);
    setDepartmentId(getNestedObject(detail, ["department", "id"]) || undefined);
    setCategoryId(getNestedObject(detail, ["category", "id"]) || undefined);
  }, [detail, reset]);

  const onSubmit = data => {
    dispatch(
      productActions.update(detail.id, {
        ...data,
  ...
  // =================
  if (detail == null && !loading) return <p className="text-center">No data</p>;
  if (detail === "UPC_NOT_FOUND") {
    return (
      <>
        {showModal && (
          <div
                ...
                <div className="modal-footer" style={{ padding: "0.5rem" }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      setShowModal(false);
                      history.push(
                        `/dashboard/inventories/create?upc=${match.params.upc}`
                      );
                    }}
                  >
                    Ok
                  ...
                  <div className="col-lg-6">
                    <div className="form-group focused">
                      ...
                      <div
                        ...
                        style={{ height: "80px" }}
                      >
                        {detail.expiration.map((date, index) => (
                          <span
                            key={index}
                          ...
                          >
                            ...
                            <i
                              ...
                              onClick={() =>
                                dispatch(productActions.removeExpiration(index))
                              }
                            ></i>
                          </span>
                        ))}
